...
<head>
    <meta charset="utf-8">
    <title>Employee Loan & Advance Statement</title>
    <style>
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td style="width: 50%;">
                ...
                <div class="meta-text">Employee Loan Recovery & EMI Register</div>
            </td>
            <td style="width: 50%;" class="text-right">
                <div class="report-title">Loan & Advance Statement</div>
    ...
    <table class="kpi-table">
        <tr>
            <td class="kpi-card" style="width: 25%;">
                <div class="kpi-title">Total Loans Disbursed</div>
                <div class="kpi-val">{{ $kpis['total_loans'] }}</div>
            </td>
            <td class="kpi-card" style="width: 25%;">
                <div class="kpi-title">Total Principal Disbursed</div>
                <div class="kpi-val" style="color: #1e3a8a;">₹{{ number_format($kpis['total_principal'], 2) }}</div>
            </td>
            <td class="kpi-card" style="width: 25%;">
                <div class="kpi-title">Total Recovered</div>
                <div class="kpi-val" style="color: #166534;">₹{{ number_format($kpis['total_repaid'], 2) }}</div>
            </td>
            <td class="kpi-card" style="width: 25%;">
                <div class="kpi-title">Total Outstanding Balance</div>
                <div class="kpi-val" style="color: #991b1b;">₹{{ number_format($kpis['total_outstanding'], 2) }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="data-table">
        ...
        <tbody>
            @foreach($rows as $r)
            <tr>
                <td><strong>{{ $r['loan_ref'] }}</strong></td>
                <td>{{ $r['employee_code'] }}</td>
                <td>{{ $r['employee_name'] }}</td>
                <td>{{ $r['client_name'] }}</td>
                <td>{{ $r['loan_type'] }}</td>
                <td class="text-right">₹{{ number_format((float)$r['principal_amount'], 2) }}</td>
                <td class="text-right">₹{{ number_format((float)$r['monthly_emi'], 2) }}</td>
                <td class="text-right">₹{{ number_format((float)$r['total_repaid'], 2) }}</td>
                <td class="text-right"><strong style="color: #991b1b;">₹{{ number_format((float)$r['remaining_balance'], 2) }}</strong></td>
                <td>{{ $r['status'] }}</td>
                <td>{{ $r['start_date'] }}</td>
            </tr>
            @endforeach
