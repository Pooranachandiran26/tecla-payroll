...
export default function UploadHistory({ 
  ...
  const filteredDetailRows = (selectedBatchDetails?.rows || []).filter(row => {
    const isSuccess = row.status === 'ready' || row.status === 'valid' || row.status === 'success' || !row.error_message;
    const isError = row.status === 'error' || row.status === 'invalid' || row.status === 'failed' || (row.error_message && row.status !== 'ready');

    if (detailFilter === 'ready' && !isSuccess) return false;
    if (detailFilter === 'error' && !isError) return false;
  ...
  const totalDetailPages = Math.ceil(filteredDetailRows.length / detailItemsPerPage);
  const paginatedDetailRows = filteredDetailRows.slice((detailPage - 1) * detailItemsPerPage, detailPage * detailItemsPerPage);
                    ...
                    <tr key={batch.id} className="hover:bg-gray-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <a
                            href={route('employees.bulk-upload.history.download-success', { batchId: batch.id })}
                          ...
                          <a
                            href={batch.error_count > 0 ? route('employees.bulk-upload.history.download-errors', { batchId: batch.id }) : '#'}
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-4">
                ...
                <div className="flex items-center justify-between flex-wrap gap-3 pb-3 border-b border-gray-100">
                  <div className="flex items-center gap-1.5 bg-gray-100 p-1 rounded-xl text-xs font-semibold">
                    <button
                      onClick={() => { setDetailFilter('all'); setDetailPage(1); }}
                    ...
                    <button
                      onClick={() => { setDetailFilter('ready'); setDetailPage(1); }}
                    ...
                    >
                      Success ({(selectedBatchDetails.rows || []).filter(r => r.status === 'ready' || r.status === 'valid' || r.status === 'success' || !r.error_message).length})
                    </button>
                    <button
                      onClick={() => { setDetailFilter('error'); setDetailPage(1); }}
                    ...
                    >
                      Failure ({(selectedBatchDetails.rows || []).filter(r => r.status === 'error' || r.status === 'invalid' || r.status === 'failed' || (r.error_message && r.status !== 'ready')).length})
                    </button>
                  </div>
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden text-xs max-h-96 overflow-y-auto">
                  <table className="w-full text-left">
                    <thead className="bg-gray-100 font-bold text-gray-700 sticky top-0 bg-white shadow-2xs">
                      <tr>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {paginatedDetailRows.length > 0 ? (
                        paginatedDetailRows.map(r => {
                          const isErrorRow = r.status === 'error' || r.status === 'invalid' || r.status === 'failed';
                          return (
                            <tr key={r.id} className={isErrorRow ? 'bg-red-50/50' : 'hover:bg-gray-50/50'}>
                              <td className="py-2 px-3 font-mono font-bold text-gray-500">{r.row_no}</td>
                              <td className="py-2 px-3 font-semibold text-gray-900">{r.employee_code || '—'}</td>
                              ...
                              <td className="py-2 px-3">
                                <span className={`px-2 py-0.5 rounded text-[0.65rem] font-bold ${
                                  !isErrorRow ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                }`}>
                                  {!isErrorRow ? 'SUCCESS' : 'FAILURE'}
                                </span>
                              </td>
                              <td className="py-2 px-3 text-xs font-medium">
                                <span className={isErrorRow ? 'text-red-700 font-semibold' : 'text-green-700'}>
                ...
                {totalDetailPages > 1 && (
                  <div className="flex items-center justify-between pt-2 text-xs">
                    <span className="text-gray-500 font-medium">
                      Showing {((detailPage - 1) * detailItemsPerPage) + 1} to {Math.min(detailPage * detailItemsPerPage, filteredDetailRows.length)} of {filteredDetailRows.length} employees
                    </span>
                    <div className="flex items-center gap-1.5">
                      <button
                        disabled={detailPage === 1}
                        onClick={() => setDetailPage(p => Math.max(1, p - 1))}
                        className="px-3 py-1 rounded-lg border border-gray-300 bg-white font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed shadow-2xs"
                      >
                        Previous
                      </button>
