                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                        <input type="number" className={`form-control ${errors.lopBasis ? `is-${errors.lopBasis.type || 'error'}` : ''}`}
                          ...
                          onChange={e => {
                            const val = e.target.value;
                            handleInputChange('lopBasis', val);
                            const clientLop = activeClientDefaults?.lopBasisDays !== undefined && activeClientDefaults?.lopBasisDays !== null && activeClientDefaults?.lopBasisDays !== '' 
                              ? String(activeClientDefaults.lopBasisDays).replace(/\D/g, '') 
                              : '26';
                            const valDigits = String(val || '').replace(/\D/g, '');
                            const isOverridden = valDigits !== '' && valDigits !== clientLop;
                            setOverrides(prev => ({ ...prev, lop: isOverridden }));
                          }}
