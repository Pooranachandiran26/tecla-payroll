...
export default function PortalSection({ formData, onChange, hook }) {
  ...
  return (
    <>
      {/* Client Portal Access — hidden for In-House */}
      {!isInhouse && (
        <>
      ...
      {/* Customizable Client Branding & Payslip Customization Section — always visible */}
      <div style={{ marginTop: isInhouse ? '0' : '1.5rem', borderTop: isInhouse ? 'none' : '1px solid var(--border-color)', paddingTop: isInhouse ? '0' : '1.5rem' }}>
        <div className="section-header">
          <div className="section-icon">{isInhouse ? <Palette size={18} /> : '🎨'}</div>
          <h3>{isInhouse ? 'Entity Branding & Payslip Customization' : 'Client Branding & Payslip Customization'}</h3>
        </div>
        ...
        <div className="form-group" style={{ marginTop: '1rem' }}>
          <label>{isInhouse ? 'Entity Logo (for Payslip Branding)' : 'Client Logo (for Payslip & Portal Branding)'}</label>
