    ...
    function renderHeader() {
      ...
      if (editBtn) {
        editBtn.setAttribute('href', `/clients/create?id=1`);
      }

      // Update Generate Invoice link dynamically
      const invoiceBtn = document.querySelector('a[href^="#"]');
      if (invoiceBtn) {
        invoiceBtn.setAttribute('href', `#`);
      }
    }
        ...
        clientCandList.forEach(cand => {
          tbody.innerHTML += `
            <tr>
              ...
              <td><a href="/employees/1" class="btn btn-secondary btn-xs">View Profile</a></td>
            </tr>
          `;
        });
      } else {
