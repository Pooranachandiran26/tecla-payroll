    ...
    // Run initialization immediately (DOM is already ready when loaded dynamically)
    (function initPage() {
      // Store initial emp type for change detection
      var empTypeEl = document.getElementById('emp-type');
      if (empTypeEl) previousEmpType = empTypeEl.value;

      // Sync saved client statutory settings from localStorage into CLIENT_STATUTORY_DEFAULTS
      if (typeof syncClientDefaultsFromStorage === 'function') syncClientDefaultsFromStorage();

      // Apply mode from URL — must run before calculateGross/tds
      if (typeof applyFormMode === 'function') applyFormMode();
      if (typeof onTdsToggleChange === 'function') onTdsToggleChange();

      // Initialize Aadhaar masked display
      aadhaarRawValue = '123456789012';
      var aadhaarEl = document.getElementById('aadhaar');
      if (aadhaarEl) {
        aadhaarEl.value = '';
        aadhaarEl.placeholder = 'Click to reveal/edit';
      }
      var aadhaarMasked = document.getElementById('aadhaar-masked');
      if (aadhaarMasked) {
        aadhaarMasked.textContent = '••••••••9012';
        aadhaarMasked.style.display = 'block';
      }

      // Sync account holder name initially
      var holderEl = document.getElementById('account-holder');
      if (holderEl) holderEl.dataset.synced = 'true';

      // Initialize branch dropdown
      var initialClient = document.getElementById('client-partner');
      if (initialClient && initialClient.value && typeof populateBranchDropdown === 'function') {
        populateBranchDropdown(initialClient.value);
      }
    })();
